...
const Nav = () => {
  ...
  useEffect(() => {
    ...
    // Clean up GSAP timeline on component unmount
    return () => {
    ...
    <nav className="flex items-center gap-0 md:gap-8">
      ...
      <div className="z-50 items-center hidden gap-8 list-none md:flex text-whiteText">
        {navLinks.map((navlink, index) => (
          <li
            key={index}
            className={`relative px-4 py-2 rounded-3xl font-semibold ${
              isActive(navlink.path) ? "bg-[#0D8F41]" : ""
            }`}
            ref={isActive(navlink.path) ? activeRef : null}
          >
            <Link to={navlink.path}>{navlink.name}</Link>
            {isActive(navlink.path) && (
              <div
                className="absolute inset-0 bg-[#0D8F41] opacity-75 rounded-3xl"
                style={{ width: "0%" }}
              />
            )}
          </li>
